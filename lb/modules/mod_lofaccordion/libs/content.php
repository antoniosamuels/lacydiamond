 ...
 */
if( !class_exists('LofGroupSimpleContent') ){ 
	class LofGroupSimpleContent extends LofGroupSimpleBase{
		 ...
		 * @return array;
		 */
		public  function __getListJLOneSix( &$params )	{
			...
			$thumbWidth    = (int)$params->get( 'thumbnail_width', 80 );
			$thumbHeight   = (int)$params->get( 'thumbnail_height', 30 );
			$imageHeight   = (int)$params->get( 'main_height', 300 ) ;
			$imageWidth    = (int)$params->get( 'main_width', 960 ) ;
			...
			foreach ($items as $key => &$item) {
				...
				$item->catlink =JRoute::_(ContentRoute::category($item->catslug));
		 ...
		 * @return Array
		 */
		public function __getListJLOneFive( $params ){
			global $mainframe; 
			...
			$titleMaxChars = $params->get( 'title_max_chars', '100' );
			$descriptionMaxChars = $params->get( 'description_max_chars', 100 );
			$condition     = self::_buildConditionQuery( $params );
			$isThumb       = $params->get( 'auto_renderthumb',1);
			$ordering      = $params->get( 'ordering', 'created_asc');
			$limit 	       = $params->get( 'limit_items', 4 );
			...
			foreach( $data as $key => $item ){	 
				...
				$item->date = JHtml::_('date', $item->created, JText::_('DATE_FORMAT_LC2')); 
				$mainframe->triggerEvent( 'onPrepareContent', array (& $item, & $params, 0)); 			
